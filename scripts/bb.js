...
export async function main(ns) {
	...
	for (; ;) {
		...
		for (const skill of ALLOWED_SKILL_UPGRATES) {
			...
			if (skill == 'Short-Circuit' && ns.bladeburner.getSkillLevel(skill) >= 25) continue;
			if (skill == 'Cloak' && ns.bladeburner.getSkillLevel(skill) >= 25) continue;
			if (skill == 'Digital Observer' && ns.bladeburner.getSkillLevel(skill) >= 500) continue;
			if (skill == "Blade's Intuition" && ns.bladeburner.getSkillLevel(skill) >= 500) continue;
		...
		if (chaos > 50) {
			while (chaos > 15 && ns.bladeburner.getActionCountRemaining("Operations", "Stealth Retirement Operation") > 0) {
				const cur = ns.bladeburner.getCurrentAction();
				if (cur.name != 'Diplomacy') {
					ColorPrint('white', 'Setting action to ', 'yellow', 'Stealth Retirement Operation', 'white', ' to lower chaos');
					ns.bladeburner.startAction('Operations', 'Stealth Retirement Operation');
				}
				await ns.sleep(1000);
...
function GetPossibleTasks(ns, type, getter) {
	...
	const allowed = ["Investigation", 'Assassination', "Sting Operation", "Raid", "Undercover Operation", 'Undercover Operation', "Stealth Retirement Operation"];
	//const allowed = ["Investigation", 'Assassination', 'Undercover Operation'];
