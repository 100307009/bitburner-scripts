...
export function IsWhacked(ns, server) {
	var so = ns.getServer(server);
	if (so.moneyAvailable > so.moneyMax*0.01) return false;
	if (so.hackDifficulty > so.minDifficulty) return false;
	return true;
}
...
export async function Hack(ns, server, pct, allowSpread = false, allowPartial = true, extra = 1.1) {
	...
	const so = ns.getServer(server);
	const threads = Math.floor(ns.hackAnalyzeThreads(server, so.moneyAvailable-(so.moneyMax*0.01)) * pct * extra);
	//ns.print(ns.hackAnalyzeThreads(server, so.moneyAvailable-(so.moneyMax*0.01)))
